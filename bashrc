...
alias g?='git status'
alias glog='__git_log'
alias glog*='git reflog; glog --all'
...
__docker_name () { echo $(basename $PWD)-$(stat -f '%i' .); }

alias d?='docker ps -a'
alias dup='docker run -di --name $(__docker_name) -v $PWD:/media'
alias ddown!='docker rm -f $(__docker_name)'
alias din='docker exec -it $(__docker_name) bash'
...
__on_prompt() {
	...
	PS1+="$(echo -e "\e];\w\x07")"
	PS1+="$(__ps1_prologue)\n\n"
	PS1+="$(AEC 35)$(printf %$(( $__term_width > 80 ? 80 : __term_width ))s | tr ' ' '_')\n"
